...
export const LiveWaveform = ({
  ...
  useEffect(() => {
    // If not active or no stream, clean up
    if (!active || !stream) {
    ...
    // Setup audio context and analyser for the provided stream
    ...
    return () => {
      // Cleanup audio context on effect cleanup
      if (audioContextRef.current && audioContextRef.current.state !== 'closed') {
        audioContextRef.current.close();
        audioContextRef.current = null;
      }
  ...
  }, [active, stream, fftSize, smoothingTimeConstant, onError, onStreamReady, onStreamEnd]);
